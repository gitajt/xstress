...
void Thread::process()
{
    ...
    {
        ulLastTS = time(NULL);
        debug("Server dropped connection or error creating socket, trying again!");
    ...
    if(rv==1 && ((fds[0].revents & POLLIN)  || (fds[0].revents & POLLOUT)))
    {
        ...
        if((fds[0].revents & POLLIN)==POLLIN)
        {
            ...
            if(mailObj.changeState(READ_READY)==ERR_IO)
            {
                ...
                logger.log("Closing current connection and creating a new one.");
                closeSocket();
                createSocket();
                //initMailObj();
            }
        ...
        if((fds[0].revents & POLLOUT)==POLLOUT || mailObj.state()==MAIL)
        {
            ...
            if(mailObj.state()!=FINISHED)
            {
                if((rv = mailObj.run(0)) != NO_ERR)
                {
                    if(rv == ERR_CONNECT)
                    {
                        const char * const _msg = "Error connecting to mail server";
                        debug(_msg);
                        logger.log(_msg);
                    }
                    ...
                    {
                        const char * const _msg = "Error invalid Socket";
                        debug(_msg);
                        logger.log(_msg);
                    }
                }
    ...
    if((time(NULL)-ulLastTS)>configObj->uiTimeout)
    {
        ...
        {
            if(configObj->uiLogTimeout) logger.log("Timeout!");
            uiMailsSend++;
            uiNotSent++;
            uiReportCnt++;
